...
#ifdef SUPPORT_SCREEN_ON_FCC_OP
#define SCREEN_ON_FCC_VOTER     "SCREEN_ON_FCC_VOTER"
#endif
#ifdef SUPPORT_CALL_ON_FCC_OP
#define	CALL_ON_FCC_VOTER       "CALL_ON_FCC_VOTER"
#endif
#ifdef SUPPORT_TEHRMAL_MITIGATION_WITH_FCC
#define	THERMAL_MITIGATION_FCC_VOTER    "THERMAL_MITIGATION_FCC_VOTER"
#endif
#ifdef SUPPORT_JEITA_TEMP_PATCH
#define JEITA_CUSTOM_FCC_VOTER     "JEITA_CUSTOM_FCC_VOTER"
#endif
#ifdef SUPPORT_LENUK_STEP_CHARGE
#define STEP_CHARGE_FCC_VOTER      "STEP_CHARGE_FCC_VOTER"
#endif
...
#ifdef SUPPORT_NON_STANDARD_CHARGER
#define NSC_ICL_VOTER        "NSC_ICL_VOTER"
#endif
#ifdef SUPPORT_LENUK_BOOTUP_ICL
#define BOOTUP_ICL_VOTER        "BOOTUP_ICL_VOTER"
#endif
...
#ifdef SUPPORT_TEHRMAL_MITIGATION_WITH_FCC
#define BATTCHG_THERMAL_MITIGATION_EN_VOTER        "BATTCHG_THERMAL_MITIGATION_EN_VOTER"
#endif
...
static void smbchg_ns_charger_monitor_work(struct work_struct *work)
{
	...
	ktime_t now_kt, delta_kt;
	int delta_ms, rc;
	int fcc_ma, usb_icl_ma, usb_suspend, battchg_suspend;
	...
	icl_voter = get_effective_client_locked(chip->usb_icl_votable);
	pr_smb(PR_STATUS, "voter: fcc = %d, icl = %d, icl_id = %s, usb_suspend = %d, battchg_suspend = %d\n",
				fcc_ma, usb_icl_ma, icl_voter, usb_suspend, battchg_suspend);
	...
	if (smbchg_check_ns_charger(chip)) {
		...
		if (smbchg_check_charger_usbin(chip) < NSC_USBIN_UV) {
			...
			battchg_suspend = get_effective_result_locked(chip->battchg_suspend_votable);
			icl_voter= get_effective_client_locked(chip->usb_icl_votable);
			pr_smb(PR_STATUS, "nsc_icl = %d, fcc = %d, icl = %d, icl_id = %s, usb_suspend = %d, battchg_suspend = %d, check_count = %d\n",
						chip->nsc_icl_ma, fcc_ma, usb_icl_ma, icl_voter, usb_suspend, battchg_suspend, chip->nsc_check_count);

			if ((((chip->nsc_icl_ma >= usb_icl_ma) && (!icl_voter || strcmp(icl_voter, NSC_ICL_VOTER) != 0)) || usb_suspend || battchg_suspend) && chip->nsc_check_count)
				schedule_delayed_work(&chip->ns_charger_monitor_work,
							msecs_to_jiffies(NS_CHARGER_USBIN_CHECK_MS));
			else {
				rc = vote(chip->usb_icl_votable, NSC_ICL_VOTER, true, chip->nsc_icl_ma);
...
static void smbchg_jeita_temp_monitor_work(struct work_struct *work)
{
	...
	const char *fcc_voter = get_effective_client_locked(chip->fcc_votable);
	int usb_icl_ma = get_effective_result_locked(chip->usb_icl_votable);
	const char *usb_icl_voter = get_effective_client_locked(chip->usb_icl_votable);
	int usb_suspend = get_effective_result_locked(chip->usb_suspend_votable);
	const char *usb_suspend_voter = get_effective_client_locked(chip->usb_suspend_votable);
	int battchg_suspend = get_effective_result_locked(chip->battchg_suspend_votable);
	const char *battchg_suspend_voter = get_effective_client_locked(chip->battchg_suspend_votable);

	pr_smb(PR_STATUS, "fcc = %d, icl = %d, usb_suspend = %d, battchg_suspend = %d  (%s, %s, %s, %s)\n",
			fcc_ma, usb_icl_ma, usb_suspend, battchg_suspend,
			fcc_voter, usb_icl_voter, usb_suspend_voter, battchg_suspend_voter);
#ifdef SUPPORT_CALL_ON_FCC_OP
	if (g_call_status){
